...
static Statements* parse_statements(TokenBuf* tokens);
static bool token_is_list_op(Token token);
static CommandList parse_list(TokenBuf* tokens);
static Command* parse_command(TokenBuf* tokens);
...
SyntaxTree parser_parse(Parser* parser) {
    ...
    Statements* result = parse_statements(&temp_tokens);
    ...
    BUF_FREE(tokens);
    return (SyntaxTree){.root = result};
}

static Statements* parse_statements(TokenBuf* tokens) {
    Statements* result = statements_new();
    CommandList list = parse_list(tokens);
...
static CommandList parse_list(TokenBuf* tokens) {
    ...
    CommandList commands = command_list_new();
    Command* command = parse_command(tokens);
    BUF_PUSH(&commands.commands, command);
    while (tokens->len > 0 && token_is_list_op(tokens->ptr[0])) {
        Op op = tokens->ptr[0].type == TOKEN_TYPE_AMP_AMP ? OP_AND : OP_OR;
        BUF_PUSH(&commands.ops, op);
        BUF_SHIFT(tokens, 1);
...
static Command* parse_command(TokenBuf* tokens) {
    ...
    if (argv.len == 0) {
        if (tokens->ptr[0].type == TOKEN_TYPE_LPAREN) {
            // subshell
            BUF_SHIFT(tokens, 1);
            Statements* statements = parse_statements(tokens);
